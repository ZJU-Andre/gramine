...
// --- Vector Add Handler (unchanged, timing is in launch_vector_add_cuda) ---
static void handle_vector_add_request(const vector_add_request_payload_t* req, vector_add_response_payload_t* resp) {
    ...
    if (req->array_len_elements == 0 || req->array_len_elements > VECTOR_ARRAY_MAX_ELEMENTS) {
        resp->status = -EINVAL; resp->array_len_elements = 0; return;
    }
    size_t data_sz = req->array_len_elements * sizeof(float);
    float *b = (float*)malloc(data_sz), *c = (float*)malloc(data_sz), *a = (float*)malloc(data_sz);
    if (!b || !c || !a) { resp->status = -ENOMEM; goto cleanup_va; }

    if (libos_aes_gcm_decrypt(g_shared_enclave_aes_key, req->iv_b, req->masked_data_b, data_sz, req->tag_b, (unsigned char*)b, NULL, 0) != 0 ||
        libos_aes_gcm_decrypt(g_shared_enclave_aes_key, req->iv_c, req->masked_data_c, data_sz, req->tag_c, (unsigned char*)c, NULL, 0) != 0) {
        resp->status = -EPERM; goto cleanup_va; 
    }
    int cuda_err_code = 0; const char* cuda_err_str = NULL;
    // launch_vector_add_cuda is expected to have internal timing and print it.
    if (launch_vector_add_cuda(a, b, c, req->array_len_elements, &cuda_err_code, &cuda_err_str) != 0) {
    ...
    if (libos_aes_gcm_encrypt(g_shared_enclave_aes_key, resp->iv_a, (unsigned char*)a, data_sz, resp->masked_data_a, resp->tag_a, NULL, 0) != 0) {
        resp->status = -EPERM; goto cleanup_va; 
    }
    resp->status = 0; resp->array_len_elements = req->array_len_elements;
cleanup_va:
...
static int handle_ort_status(OrtStatus* status, const char* op_name) { 
    if (status) {
...
static int init_onnx_runtime(const char* model_path) { 
...
static void shutdown_onnx_runtime() { 
...
static void handle_onnx_inference_request(const onnx_inference_request_payload_t* req, onnx_inference_response_payload_t* resp) {
    printf("SHARED_SERVICE_LOG: Handling ONNX_INFERENCE_REQUEST (input size: %u bytes).\n", req->input_tensor_size_bytes);
    ...
    unsigned char* p_input = (unsigned char*)malloc(req->input_tensor_size_bytes);
    if (!p_input) { resp->status = -ENOMEM; goto cleanup_onnx; }
    if (libos_aes_gcm_decrypt(g_shared_enclave_aes_key, req->iv, req->masked_input_tensor, req->input_tensor_size_bytes, req->tag, p_input, NULL, 0) != 0) {
    ...
    OrtStatus* ort_status = g_ort_api->CreateTensorWithDataAsOrtValue(g_ort_allocator, p_input, req->input_tensor_size_bytes, input_shape, 4, ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT, &in_tensor);
    if (handle_ort_status(ort_status, "CreateTensorInput") != 0) goto cleanup_onnx;
    ...
    const float* out_data_ptr;
    ort_status = g_ort_api->GetTensorData(out_tensor, (const void**)&out_data_ptr);
    ...
    if (libos_aes_gcm_encrypt(g_shared_enclave_aes_key, resp->iv, (unsigned char*)out_data_ptr, out_bytes, resp->masked_output_tensor, resp->tag, NULL, 0) != 0) {
        resp->status = -EPERM; goto cleanup_onnx;
    }
    resp->output_tensor_size_bytes = out_bytes; resp->status = 0;
cleanup_onnx:
    if (g_ort_api) { if (in_tensor) g_ort_api->ReleaseValue(in_tensor); if (out_tensor) g_ort_api->ReleaseValue(out_tensor); }
    free(p_input);
...
static void handle_gemm_request(const gemm_request_payload_t* req, gemm_response_payload_t* resp) {
    printf("SHARED_SERVICE_LOG: Handling GEMM_REQUEST (M=%d, N=%d, K=%d)\n", req->M, req->N, req->K);
    ...
    float *pA=NULL, *pB=NULL, *pC_host=NULL;
    float *dA=NULL, *dB=NULL, *dC=NULL;
    cudaEvent_t start_event, stop_event; float gpu_time_ms = 0.0f;

    pA = (float*)malloc(req->matrix_a_size_bytes);
    pB = (float*)malloc(req->matrix_b_size_bytes);
    resp->matrix_c_size_bytes = (size_t)req->M * req->N * sizeof(float);
    if (resp->matrix_c_size_bytes > MAX_GEMM_MATRIX_SIZE_BYTES) { resp->status = -EFBIG; goto cleanup_gemm; }
    pC_host = (float*)malloc(resp->matrix_c_size_bytes);

    if(!pA || !pB || !pC_host) { resp->status = -ENOMEM; goto cleanup_gemm; }

    if (libos_aes_gcm_decrypt(g_shared_enclave_aes_key, req->iv_a, req->masked_matrix_a, req->matrix_a_size_bytes, req->tag_a, (unsigned char*)pA, NULL, 0) != 0 ||
        libos_aes_gcm_decrypt(g_shared_enclave_aes_key, req->iv_b, req->masked_matrix_b, req->matrix_b_size_bytes, req->tag_b, (unsigned char*)pB, NULL, 0) != 0) {
        resp->status = -EPERM; goto cleanup_gemm;
    }

    if (cudaMalloc((void**)&dA, req->matrix_a_size_bytes) != cudaSuccess ||
        cudaMalloc((void**)&dB, req->matrix_b_size_bytes) != cudaSuccess ||
        cudaMalloc((void**)&dC, resp->matrix_c_size_bytes) != cudaSuccess) {
        resp->status = -ENOMEM; goto cleanup_gemm_gpu;
    }
    if (cudaMemcpy(dA, pA, req->matrix_a_size_bytes, cudaMemcpyHostToDevice) != cudaSuccess ||
        cudaMemcpy(dB, pB, req->matrix_b_size_bytes, cudaMemcpyHostToDevice) != cudaSuccess) {
    ...
    if (cudaMemcpy(pC_host, dC, resp->matrix_c_size_bytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
        resp->status = -EIO; goto cleanup_gemm_gpu;
    }
    generate_iv_for_response(resp->iv_c, GCM_IV_SIZE_BYTES);
    if (libos_aes_gcm_encrypt(g_shared_enclave_aes_key, resp->iv_c, (unsigned char*)pC_host, resp->matrix_c_size_bytes, resp->masked_matrix_c, resp->tag_c, NULL, 0) != 0) {
