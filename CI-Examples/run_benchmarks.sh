...
run_sgx_shared_vector_add() {
    log_message "Running Gramine SGX Shared Vector Add (Size: $1)"
    local size="$1"
    log_message "Running Gramine SGX Shared Vector Add (Size: $1)"
    local size="$1"
    local app_name="vector_add"
    local client_app_name="client_app" # The vector_add client
    local result_prefix="${RESULTS_DIR}/${app_name}_sgx_shared_size${size}_${TIMESTAMP}"
    local server_log="${SHARED_ENCLAVE_DIR}/logs/${app_name}_server_${TIMESTAMP}.log"
    local client_log="${CLIENT_ENCLAVE_DIR}/logs/${app_name}_client_size${size}_${TIMESTAMP}.log"
    local server_pid_file="${SHARED_ENCLAVE_DIR}/logs/server.pid"
    ...
    log_message "Running Client Application for ${app_name} (Size: ${size})..."
    # The client app for vector_add doesn't take size as arg, it uses fixed size from header
    # If it were to take size, it would be: gramine-sgx client-enclave.manifest.sgx "$size"
    # For now, client_app internally uses VECTOR_ARRAY_DEFAULT_ELEMENTS
    echo "Command: /usr/bin/time -v -o \"${result_prefix}_client.time\" gramine-sgx client-enclave.manifest.sgx"
    (cd "${CLIENT_ENCLAVE_DIR}" && \
        /usr/bin/time -v -o "${result_prefix}_client.time" \
        gramine-sgx client-enclave.manifest.sgx > "${client_log}" 2>&1)
    client_exit_code=$?

    log_message "Stopping Shared Enclave Service for ${app_name}..."
    ...
    log_message "Gramine SGX Shared Vector Add (Size: ${size}) finished. Client log: ${client_log}"
...
run_sgx_shared_onnx() {
    log_message "Running Gramine SGX Shared ONNX Inference (${ONNX_MODEL_NAME})"
    log_message "Running Gramine SGX Shared ONNX Inference (${ONNX_MODEL_NAME})"
    local app_name="onnx_inference"
    local client_app_name="client_app_onnx" # Specific ONNX client
    local result_prefix="${RESULTS_DIR}/${app_name}_sgx_shared_${TIMESTAMP}"
    local server_log="${SHARED_ENCLAVE_DIR}/logs/${app_name}_server_${TIMESTAMP}.log"
    local client_log="${CLIENT_ENCLAVE_DIR}/logs/${app_name}_client_${TIMESTAMP}.log"
    local server_pid_file="${SHARED_ENCLAVE_DIR}/logs/server.pid" # Re-using server.pid
    ...
    log_message "Running Client Application for ${app_name}..."
    echo "Command: /usr/bin/time -v -o \"${result_prefix}_client.time\" gramine-sgx client-enclave-onnx.manifest.sgx"
    (cd "${CLIENT_ENCLAVE_DIR}" && \
        /usr/bin/time -v -o "${result_prefix}_client.time" \
        gramine-sgx client-enclave-onnx.manifest.sgx > "${client_log}" 2>&1)
    client_exit_code=$?
    ...
    log_message "Gramine SGX Shared ONNX Inference finished. Client log: ${client_log}"
    echo "Timing data (overall client process): ${result_prefix}_client.time"
}
...
run_sgx_shared_gemm() {
    log_message "Running Gramine SGX Shared GEMM (M=$1, N=$2, K=$3)"
    local M="$1" N="$2" K="$3"
    log_message "Running Gramine SGX Shared GEMM (M=$1, N=$2, K=$3)"
    local M="$1" N="$2" K="$3"
    local app_name="gemm"
    local client_app_name="client_app_gemm" # Specific GEMM client
    local result_prefix="${RESULTS_DIR}/${app_name}_sgx_shared_M${M}_N${N}_K${K}_${TIMESTAMP}"
    local server_log="${SHARED_ENCLAVE_DIR}/logs/${app_name}_server_${TIMESTAMP}.log"
    local client_log="${CLIENT_ENCLAVE_DIR}/logs/${app_name}_client_M${M}N${N}K${K}_${TIMESTAMP}.log"
    local server_pid_file="${SHARED_ENCLAVE_DIR}/logs/server.pid" # Re-using server.pid
    ...
    log_message "Running Client Application for ${app_name} (M=${M}, N=${N}, K=${K})..."
    # Assuming client_app_gemm takes M, N, K as arguments or uses fixed values.
    # For this example, client_app_gemm uses fixed values GEMM_M, GEMM_N, GEMM_K
    echo "Command: /usr/bin/time -v -o \"${result_prefix}_client.time\" gramine-sgx client-enclave-gemm.manifest.sgx"
    (cd "${CLIENT_ENCLAVE_DIR}" && \
        /usr/bin/time -v -o "${result_prefix}_client.time" \
        gramine-sgx client-enclave-gemm.manifest.sgx > "${client_log}" 2>&1)
    client_exit_code=$?
    ...
    log_message "Gramine SGX Shared GEMM finished. Client log: ${client_log}"
    echo "Timing data (overall client process): ${result_prefix}_client.time"
}
...
main() {
    ...
    run_sgx_shared_vector_add "$VECTOR_SIZE_ELEMENTS" && echo "SGX Shared Vector Add: OK" || echo "SGX Shared Vector Add: FAILED"
    ...
    run_sgx_shared_onnx && echo "SGX Shared ONNX: OK" || echo "SGX Shared ONNX: FAILED"
    ...
    run_sgx_shared_gemm "$GEMM_M" "$GEMM_N" "$GEMM_K" && echo "SGX Shared GEMM: OK" || echo "SGX Shared GEMM: FAILED"
