...
typedef struct {
    ...
    // Payload for the input tensor.
    unsigned char masked_input_tensor[MAX_ONNX_INPUT_SIZE_BYTES]; 
...
typedef struct {
    ...
    unsigned char iv[GCM_IV_SIZE_BYTES];
    unsigned char tag[GCM_TAG_SIZE_BYTES];
    // Payload for the output tensor.
    unsigned char masked_output_tensor[MAX_ONNX_OUTPUT_SIZE_BYTES];
